...
class AsyncCLIPFetcher:
    ...
    async def fetch(self, source: str, use_cache: bool = True, validate: bool = True) -> Dict[str, Any]:
        """
        ...
        if self._is_url(source):
            return await self.fetch_from_url(source, use_cache=use_cache, validate=validate)
        else:
            return await self.fetch_from_file(source, validate=validate)
    
    async def fetch_from_url(self, url: str, use_cache: bool = True, validate: bool = True) -> Dict[str, Any]:
        """
        ...
        if self.cache_enabled and use_cache and self.cache:
            ...
            if cached_object is not None:
                if not validate or self._is_valid_clip_structure(cached_object):
                    ...
                    logger.warning(f"Cached object for {url} failed validation, refetching")
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                async with aiohttp.ClientSession(timeout=self.timeout) as session:
                    ...
                    aiohttp.ClientTimeout, json.JSONDecodeError, ValueError) as e:
                last_exception = e
        ...
        raise AsyncCLIPFetchError(f"Failed to fetch CLIP object from {url}: {str(last_exception)}")
    
    async def fetch_from_file(self, file_path: str, validate: bool = True) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            clip_object = await loop.run_in_executor(None, load_json_from_path, file_path)
            ...
            raise AsyncCLIPFetchError(f"Failed to load CLIP object from {file_path}: {str(e)}")
    
    async def fetch_batch(self, sources: List[str], use_cache: bool = True, validate: bool = True, 
                         max_concurrent: int = 10) -> List[Union[Dict[str, Any], Exception]]:
        """
        ...
        async def fetch_with_semaphore(source: str) -> Union[Dict[str, Any], Exception]:
            async with semaphore:
                try:
                    return await self.fetch(source, use_cache=use_cache, validate=validate)
                except Exception as e:
        ...
        logger.info(f"Batch fetch completed: {successful_count}/{len(sources)} successful")
        
        return results
    
    async def fetch_multiple(self, sources: List[str], use_cache: bool = True, validate: bool = True,
    ...
    async def discover_from_directory(self, directory: str, recursive: bool = True) -> List[str]:
        """
    ...
    async def prefetch_urls(self, urls: List[str], use_cache: bool = True, 
        ...
        if urls_to_fetch:
            fetch_results = await self.fetch_batch(urls_to_fetch, use_cache=use_cache, max_concurrent=max_concurrent)
            
            for i, result in enumerate(fetch_results):
                if isinstance(result, Exception):
                    results['failed'].append({'url': urls_to_fetch[i], 'error': str(result)})
                else:
                    results['successful'].append(urls_to_fetch[i])
        
        results['total_time'] = time.time() - start_time
        
        logger.info(f"Async prefetch completed: {len(results['successful'])} successful, "
