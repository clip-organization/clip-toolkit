...
class CLIPCache:
    ...
    def set(
        ...
        if key not in self.memory_cache and len(self.memory_cache) >= self.max_memory_entries:
    ...
    def _determine_max_age(
        ...
        if http_headers:
            ...
            if 'max-age=' in cache_control:
                try:
                    max_age_match = cache_control.split('max-age=')[1].split(',')[0].strip()
    ...
    def _should_cleanup(self) -> bool:
        ...
        return (datetime.now() - self.last_cleanup).total_seconds() > self.cleanup_interval_seconds
