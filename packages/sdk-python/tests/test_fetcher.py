...
class TestCLIPFetcher:
    ...
    def test_fetch_multiple_success(self, tmp_path):
        ...
        clip1_data = {
            "@context": "https://clipprotocol.org/v1",
            "type": "Venue",
            "id": "clip:test:venue:1",
        }
        clip2_data = {
            "@context": "https://clipprotocol.org/v1",
            "type": "Device",
            "id": "clip:test:device:2",
        }
    ...
    def test_is_likely_clip_object(self):
        ...
        # Valid CLIP objects
        assert (
            fetcher._is_likely_clip_object(
                {"@context": "https://clipprotocol.org/v1", "type": "Venue"}
            )
            is True
        )
        ...
        assert (
            fetcher._is_likely_clip_object(
                {"type": "Device", "id": "clip:test:device:123"}
            )
            is True
        )

        # Invalid objects
        assert fetcher._is_likely_clip_object({"name": "Not a CLIP object"}) is False
